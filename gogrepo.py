...
def cmd_download(savedir, skipextras, skipgames, dryrun, id):
    ...
    for item in sorted(items, key=lambda g: g.title):
        ...
        if not dryrun:
            ...
            info_file_contents = None
            if os.path.exists(os.path.join(item_homedir, INFO_FILENAME)):
        ...
        if not dryrun:
            if item.serial != '':
                ...
                serial_file_contents = None
                if os.path.exists(os.path.join(item_homedir, SERIAL_FILENAME)):
